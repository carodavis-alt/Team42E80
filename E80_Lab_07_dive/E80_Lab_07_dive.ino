...
void setup() {
  ...
  const int num_depth_waypoints = 7; //6
  double depth_waypoints [] = { 0.5, 1, 1.5, 2.5, 3, 0 };  // 1.5,2, 2.5,3  listed as z0,z1,... etc.
...
void loop() {
  ...
  if ( currentTime-depth_control.lastExecutionTime > LOOP_PERIOD ) {
    ...
    if ( depth_control.diveState ) {      // DIVE STATE //
      ...
      motor_driver.drive(0,-depth_control.uV,-depth_control.uV); // (A, B, C)
    }
