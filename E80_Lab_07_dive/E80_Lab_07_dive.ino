...
void setup() {
  ...
  int diveDelay = 10*1000; // how long robot will stay at depth waypoint before continuing (ms)

  const int num_depth_waypoints = 6; //6
  double depth_waypoints [] = { 0.5, 1, 1.5, 2.5, 3 };  // 1.5,2, 2.5,3  listed as z0,z1,... etc.
  ...
  delay(120000); // 120 second delay before robot begins
}
...
void loop() {
  ...
  if ( currentTime-depth_control.lastExecutionTime > LOOP_PERIOD ) {
    ...
    if ( depth_control.diveState ) {      // DIVE STATE //
      ...
      motor_driver.drive(depth_control.uV,0,depth_control.uV); // (A, B, C)
    }
